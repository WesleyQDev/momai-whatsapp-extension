  ...
  return (
    <div className="flex-1 flex flex-col p-8 bg-bg overflow-y-auto">
      <header className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-text mb-1">Lembretes</h1>
          <p className="text-text-muted text-sm">Gerencie seus agendamentos e tarefas</p>
        </div>
        <button
          onClick={fetchReminders}
          className="p-2 rounded-lg bg-card border border-border hover:border-accent/40 text-text transition-all"
        >
          <svg
            width="20"
            height="20"
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center p-12 bg-card rounded-2xl border border-dashed border-border">
          ...
          <h3 className="text-lg font-medium text-text mb-2">Nenhum lembrete</h3>
          <p className="text-text-muted max-w-xs">
            Peça para a MomAI agendar algo como: "Me lembre de beber água a cada 2 horas".
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {reminders.map((reminder) => (
            <div
              key={reminder.id}
              className={`p-5 rounded-2xl border transition-all bg-card ${reminder.is_active ? 'border-accent/40 shadow-lg shadow-accent/5' : 'border-border opacity-80'}`}
            >
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full ${reminder.is_active ? 'bg-accent animate-pulse' : 'bg-text-muted'}`}
                  ></div>
                  <h3
                    className={`font-bold ${reminder.is_active ? 'text-text' : 'text-text-muted'}`}
                  >
                    {reminder.title}
                  </h3>
                </div>
              ...
              <p className="text-sm text-text-muted mb-4 line-clamp-2">
                {reminder.content || 'Sem descrição'}
              </p>

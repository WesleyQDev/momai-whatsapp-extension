...
export default function FortScriptToast() {
  const [event, setEvent] = useState<{ status: 'active' | 'inactive'; timestamp: string } | null>(null)
  ...
  return (
    <div className="fixed bottom-24 right-6 z-[100] animate-in slide-in-from-bottom-4 fade-in duration-500">
      <div className={`p-4 rounded-2xl border shadow-2xl flex items-center gap-4 min-w-[320px] backdrop-blur-xl ${
        isActive 
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            ...
            <div className={`w-1.5 h-1.5 rounded-full ${isActive ? 'bg-accent animate-pulse' : 'bg-text-muted'}`} />
          </div>

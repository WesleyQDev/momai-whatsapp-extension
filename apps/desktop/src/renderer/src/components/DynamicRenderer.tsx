...
export const DynamicRenderer: React.FC<DynamicRendererProps> = ({ schema, onAction }) => {
  ...
  switch (type) {
    ...
    case 'text':
      return (
        <div
          className={`text-text ${props.variant === 'title' ? 'text-lg font-bold text-white mb-1' : props.variant === 'subtitle' ? 'text-sm font-semibold text-accent uppercase tracking-wide mb-2' : 'text-sm text-text-muted leading-relaxed'} ${props.className || ''}`}
        >
          {props.content}
        </div>
      )
    ...
    case 'input':
      return (
        <div className="flex flex-col gap-1.5 w-full">
          {props.label && (
            <label className="text-xs font-medium text-text-muted ml-1">{props.label}</label>
          )}
